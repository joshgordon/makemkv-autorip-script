...
printf '%s\n' "${drives[@]}"
echo "----------------------------"
python3 "$scriptroot/web_status.py" &
WEBPID=$!
trap "kill $WEBPID 2>/dev/null" EXIT
echo "[INFO] Status dashboard: http://$(hostname):${webport}"

if [ -n "$slackwebhook" ]; then
	drives_list=$(printf '`%s` ' "${drives[@]}")
	curl -s -o /dev/null -X POST -H 'Content-type: application/json' \
		--data "{\"text\":\"AutoRip started on \`$(hostname)\`. Monitoring drives: ${drives_list} Dashboard: http://$(hostname):${webport}\"}" \
		"$slackwebhook"
fi
